...
import kotlinx.android.synthetic.main.activity_german_body_result.*

class GermanBodyResultActivity : AppCompatActivity()
{
    @SuppressLint("SetTextI18n")
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val totalQuestions = intent.getIntExtra(GermanBodyConstants.TOTAL_QUESTIONS, 0)
        val correctAnswers = intent.getIntExtra(GermanBodyConstants.CORRECT_ANSWERS, 0)
        if (correctAnswers >= 6) {
            setContentView(R.layout.activity_german_body_result)
            window.decorView.systemUiVisibility = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
            text_view_german_body_score.text =
                "Your score is $correctAnswers out of $totalQuestions"
            button_german_body_finish.setOnClickListener {
